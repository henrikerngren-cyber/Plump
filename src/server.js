...
io.on('connection', (socket) => {
  ...
  socket.on('rejoin_game', async ({ roomId, playerName }, callback) => {
    try {
      ...
      // Om spelet precis startats (väntar på att alla ska återansluta), räkna upp
      if (room.status === 'playing' && room.playersReadyToStart !== undefined) {
      ...
      if (room.status === 'playing' && room.playersReadyToStart !== undefined &&
        ...
        room.playersReadyToStart = undefined; // återställ flaggan
        await saveRoom(room);
        // Läs om rummet från DB så vi har senaste socketIds, starta sedan
        setTimeout(async () => {
