...
async function cleanupOldRooms() {
  try {
    await pool.query(`DELETE FROM rooms WHERE updated_at < NOW() - INTERVAL '24 hours'`);
...
async function startPlaying(room, roomId) {
  ...
  const firstPlayerName = room.players.find(p => p.socketId === playOrder[0] || p.id === playOrder[0])?.name || '';
