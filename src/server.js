...
io.on('connection', (socket) => {
  ...
  socket.on('start_game', async ({ roomId }, callback) => {
    try {
      ...
      room.readyPlayerNames = []; // spelare som bekräftat att de är redo
      await saveRoom(room);
      callback({ success: true });
      io.to(roomId).emit('game_starting', sanitizeRoom(room));
      // startRound körs när alla spelare skickat player_ready_to_start
    } catch (err) {
  ...
  socket.on('rejoin_game', async ({ roomId, playerName }, callback) => {
    try {
      ...
      // Inget mer att göra här - klienten skickar player_ready_to_start när den är redo
    } catch (err) {
  ...
  // Spelare redo att starta (skickas efter rejoin_game lyckats)
  socket.on('player_ready_to_start', async ({ roomId, playerName }) => {
    try {
      const room = await getRoom(roomId);
      if (!room || room.status !== 'playing' || !room.readyPlayerNames) return;

      // Lägg till spelaren om hen inte redan är med
      if (!room.readyPlayerNames.includes(playerName)) {
        room.readyPlayerNames.push(playerName);
        await saveRoom(room);
        console.log(`player_ready_to_start: ${playerName}, ${room.readyPlayerNames.length}/${room.players.length} redo`);
      }

      // Starta när alla spelare är redo
      if (room.readyPlayerNames.length >= room.players.length) {
        room.readyPlayerNames = undefined;
        await saveRoom(room);
        console.log('Alla redo, startar omgång. socketIds:', room.players.map(p => p.name + ':' + p.socketId));
        // Vänta 300ms så att alla socket.join(roomId) hinner köras
        await new Promise(r => setTimeout(r, 300));
        const freshRoom = await getRoom(roomId);
        if (freshRoom) await startRound(freshRoom, roomId);
      }
    } catch (err) { console.error('player_ready_to_start fel:', err); }
  });
