...
io.on('connection', (socket) => {
  ...
  socket.on('start_game', async ({ roomId }, callback) => {
    try {
      ...
      // Vänta så att klienterna hinner navigera och återansluta med nya socketIds
      setTimeout(async () => {
        try {
          console.log('setTimeout triggad, läser rum:', roomId);
          const freshRoom = await getRoom(roomId);
          if (!freshRoom) {
            console.error('Rum saknas i DB:', roomId);
            return;
          }
          console.log('Startar omgång, spelare:', freshRoom.players.map(p => p.name + ':' + p.socketId));
          await startRound(freshRoom, roomId);
        } catch (err) {
          console.error('startRound fel i setTimeout:', err);
        }
      }, 3000);
    } catch (err) {
  ...
  socket.on('rejoin_game', async ({ roomId, playerName }, callback) => {
    try {
      ...
      // Spara gammalt socketId INNAN vi uppdaterar
      ...
      // Uppdatera hands med nytt socketId (använd oldSocketId)
      if (room.hands) {
      ...
      if (room.bids && room.bids[oldSocketId] !== undefined) {
        room.bids[socket.id] = room.bids[oldSocketId];
        delete room.bids[oldSocketId];
      }

      // Uppdatera tricks med nytt socketId
      if (room.tricks && room.tricks[oldSocketId] !== undefined) {
        room.tricks[socket.id] = room.tricks[oldSocketId];
        delete room.tricks[oldSocketId];
      }

      // Hitta handen med nytt socketId (eller namn som fallback)
      const myHand = room.hands?.[socket.id] || room.hands?.[player.name] || [];
      ...
      // Skicka alltid round_started så klienten får rätt state oavsett när den anslöt
      if (room.status === 'playing') {
        socket.emit('round_started', {
          roundIndex: room.currentRoundIndex,
          totalRounds: room.roundSequence?.length || 0,
          numCards: room.roundSequence?.[room.currentRoundIndex] || 0,
      ...
      // Om korten redan delats ut, skicka alltid cards_dealt till spelaren
      if (room.deckState === 'dealt' && room.status === 'playing') {
        const hand = room.hands?.[socket.id] || [];
        socket.emit('cards_dealt', {
          hand: numCards === 1 ? [] : hand,
          numCards,
          isOneCardRound: numCards === 1,
          dealer: room.currentDealer,
          roundIndex: room.currentRoundIndex,
          totalRounds: room.roundSequence?.length || 0
        });
      ...
      // Om budgivning redan pågår, skicka om bidding_started direkt till spelaren
      if (room.deckState === 'dealt' && room.status === 'playing') {
        ...
        socket.emit('bidding_started', {
          ...
          firstBidderName: room.players.find(p => p.socketId === currentBidder)?.name || '',
          biddingOrder,
          numCards,
          isOneCardRound,
          playerNames
  ...
  socket.on('shuffle_deck', async ({ roomId }, callback) => {
    try {
      const room = await getRoom(roomId);
      if (!room) return;
      // Synka currentDealer via namn vid varje åtgärd
      ...
      if (room.currentDealer !== socket.id) return;
  ...
  socket.on('cut_deck', async ({ roomId }, callback) => {
    try {
      const room = await getRoom(roomId);
      if (!room) return;
      const dealerPlayerCut = room.players.find(p => p.name === room.currentDealerName);
      if (dealerPlayerCut) room.currentDealer = dealerPlayerCut.socketId;
      if (room.currentDealer !== socket.id) return;
  ...
  socket.on('deal_cards', async ({ roomId }, callback) => {
    try {
      const room = await getRoom(roomId);
      if (!room) return;
      const dealerPlayerDeal = room.players.find(p => p.name === room.currentDealerName);
      if (dealerPlayerDeal) room.currentDealer = dealerPlayerDeal.socketId;
      if (room.currentDealer !== socket.id || room.deckState !== 'cut') return;
      
      const numCards = room.roundSequence[room.currentRoundIndex];
      // Dela ut kort - använd socketId som nyckel så vi hittar dem vid rejoin
      const isOneCardRound = numCards === 1;
      room.hands = {};
      ...
      for (let i = 0; i < numCards; i++) {
        for (const player of room.players) {
          if (!room.hands[player.socketId]) room.hands[player.socketId] = [];
          if (cardIndex < deck.length) room.hands[player.socketId].push(deck[cardIndex++]);
        }
      }
      room.deckState = 'dealt';
      room.bids = {};
      room.tricks = {};
      room.players.forEach(p => { room.tricks[p.socketId] = 0; room.tricks[p.id] = 0; });
      ...
      for (const player of room.players) {
        const hand = isOneCardRound ? [] : (room.hands[player.socketId] || []);
      ...
      // Starta budgivning - kort fördröjning så alla hinner joina socket-rummet
      setTimeout(() => startBidding(room, roomId), 500);
  ...
  socket.on('play_card', async ({ roomId, cardIndex, card: cardData }, callback) => {
    try {
      ...
      // Hämta handen - sök med socketId och namn som fallback
      const playerForHand = room.players.find(p => p.socketId === socket.id || p.id === socket.id);
      const hand = room.hands[socket.id] || (playerForHand ? room.hands[playerForHand.name] : null) || [];
      console.log('play_card: hand length:', hand.length, 'socketId:', socket.id, 'hands keys:', Object.keys(room.hands || {}));
      ...
      // Vid 1-kortsomgång spelar klienten index 0 - använd det enda kortet i handen
      const numCardsInRound = room.roundSequence[room.currentRoundIndex];
      if (numCardsInRound === 1 && hand.length > 0) resolvedIndex = 0;
